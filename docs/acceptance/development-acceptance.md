...
验收日期：2026-08-25
本轮补充开发的逐项结果以[项目补充开发完成与验收清单](project-completion-acceptance-2026-08-25.md)为准；上线环境证据继续参考[全链路完成度审计](full-stack-verification-2026-08-24.md)。
...
- 机构目录与成员生命周期、帮助工单会话、政策与来源追溯，以及对应后台处理入口。
- 城市成员/活动/机构/政策分区、成员公开关系与活动记录、分层评论和话题筛选。
- FAQ 与政策 CMS、活动签到/缺席、通知深链、账号数据导出和注销内容匿名化。
- 用户/角色、城市作用域、内容治理、活动、媒体、申请、工单、死信、审计后台。
- Outbox Worker、幂等领取、指数重试、死信、管理员告警和媒体内容安全接入。

按本轮需求，手机号登录和短信验证未开发、未调整。首页手机号弹层仍是明确的本地演示例外，使用 `localStorage`，不属于真实认证；生产认证只验收 `/login`。

OPC 认证申请与审核已从当前产品范围取消，现有历史数据库迁移仅作为兼容记录保留，不再由应用代码读取或写入。
...
- 50 项公开/响应式/可访问性浏览器 E2E 通过，16 项按认证、数据库和真实 OSS 环境跳过，0 项失败。
- 生产依赖 audit 未发现已知漏洞。
- 应用内浏览器实际打开首页、机构、城市成员、政策和帮助页面，未发现横向溢出或 Next.js 运行时错误覆盖层。
