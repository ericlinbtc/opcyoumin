...
import { ActivityReviewControls, AppealReviewControls, CityAdminControl, CommentModerationControls, MediaReviewControls, PostModerationControls, ReportReviewControls, UserRoleControl, UserStatusControls } from '@/features/admin/admin-controls';
import { ContentEditor } from '@/features/admin/content-editor';
import { requireSession } from '@/server/auth/session';
import { listManagedCityIds } from '@/server/auth/city-scope';
import { listAdminActivities, listAdminAppeals, listAdminAuditLogs, listAdminCities, listAdminComments, listAdminContent, listAdminMedia, listAdminPosts, listAdminReports, listAdminUsers } from '@/server/repositories/admin';
...
export default async function AdminSectionPage({ params }: { params: Promise<{ section: string }> }) {
  ...
  if (section === 'users') rows = (await listAdminUsers()).map((row) => <div role="row" key={row.id}><span>{row.nickname}<small>{row.id.slice(0, 8)}</small></span><span>{row.status}</span><UserRoleControl userId={row.id} role={row.role} /><span>{row.lastLoginAt?.toLocaleString('zh-CN') ?? '从未'}</span><UserStatusControls userId={row.id} status={row.status} /></div>);
