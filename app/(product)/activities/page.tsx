...
export const metadata: Metadata = createPageMetadata({ title: '城市活动｜游民', description: '发现 OPC 城市活动并报名参与。', canonical: '/activities' });
export const dynamic = 'force-dynamic';

export default async function ActivitiesPage({ searchParams }: { searchParams: Promise<{ cityId?: string; page?: string }> }) {
  const filters = await searchParams;
  const page = Math.max(1, Number(filters.page) || 1);
  const allActivities = await listPublicActivities(filters.cityId);
  const activityItems = allActivities.slice((page - 1) * 12, page * 12);
  ...
  return <main className="feature-page activity-center"><PageHero eyebrow="CITY ACTIVITIES" title="在线认识，线下连接" description="小规模、高质量、可追踪的城市活动。首发版本不包含在线支付。" count={String(allActivities.length)} unit="场开放活动" tone="activity-hero" /><section className="feature-page-body">{canCreate && <ActivityCreator cities={cities.map(({ id, name }) => ({ id, name }))} />}{session?.role === 'user' && !canCreate && <ActivityCreatorApplication requested={Boolean(capability?.requestedAt)} />}<div className="my-activity-list">{activityItems.map((activity) => <article className="my-activity-card route-activity-card" key={activity.id}><div className="activity-card-index" aria-hidden="true">{activity.date.slice(5).replace('-', '.')}</div><div><small>{activity.city} · {activity.location}</small><h3>{activity.title}</h3><p>{activity.summary}</p><div className="activity-status"><b>{activity.registered}/{activity.capacity} 人已报名</b><span>{activity.date}</span></div><Link className="directory-link" href={`/activities/${activity.id}`}>查看活动 →</Link></div></article>)}</div>{activityItems.length === 0 && <p className="empty-state">暂无已发布活动。</p>}<nav className="directory-pagination" aria-label="活动分页">{page > 1 ? <Link href={`/activities?page=${page - 1}${filters.cityId ? `&cityId=${encodeURIComponent(filters.cityId)}` : ''}`}>← 上一页</Link> : null}{page * 12 < allActivities.length ? <Link href={`/activities?page=${page + 1}${filters.cityId ? `&cityId=${encodeURIComponent(filters.cityId)}` : ''}`}>下一页 →</Link> : null}</nav></section></main>;
}
