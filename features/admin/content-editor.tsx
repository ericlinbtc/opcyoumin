...
type ContentValue = { id?: string; kind: 'knowledge' | 'insight'; slug: string; title: string; summary: string; body: string; category: string; importance: number; status: 'draft' | 'pending' | 'published' | 'hidden' | 'deleted' };
const empty: ContentValue = { kind: 'knowledge', slug: '', title: '', summary: '', body: '', category: '', importance: 1, status: 'draft' };
...
export function ContentEditor({ value = empty }: { value?: ContentValue }) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await saveManagedContent({ id: value.id, kind: data.get('kind'), slug: data.get('slug'), title: data.get('title'), summary: data.get('summary'), body: data.get('body'), category: data.get('category'), importance: Number(data.get('importance')), status: data.get('status') });
